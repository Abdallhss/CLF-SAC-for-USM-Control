...
class ReplayBufferLSTM2:
    ...
    def __init__(self, capacity, env_type):
        ...
        self.env_type = env_type
    def push(self, state, action, last_action, reward, next_state):
        if len(self.buffer) < self.capacity:
            self.buffer.append(None)
        self.buffer[self.position] = (state, action, last_action, reward, next_state)
...
class SAC:
   ...
   # Update networks given a batch of experiences
       ...
       # Main loop: collect experience in env and update/log each epoch
        for ep in range(1,num_eps):
           ...
           # Scale observation
            ...
            self.replay_buffer_LSTM.push(episode_state, episode_action, episode_last_action, episode_reward, episode_next_state)
    ...
    def load_network(self, env_type='sim'):
        ...
        # Set up optimizers for trainable weights log_alpha/log_beta
        self.alpha_optimizer = Adam([self.log_alpha], lr=self.lr/4)
        self.beta_optimizer = Adam([self.log_beta], lr=self.lr/4)
    ...
    def plot_eval(self,states,speeds, actions):
        ...
        ax11.set_ylabel('Admittance Phase [$^\circ$]',fontweight='bold',fontsize=16);
        ax1.legend();
        ax11.legend();
