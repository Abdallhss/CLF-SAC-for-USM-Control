...
class X2hNetwork(nn.Module):
    ...
    def __init__(self, obs_dim, hidden_dim, LSTM_out_dim, activation=F.relu):
        ...
        self.out_layer = nn.Linear(hidden_dim, LSTM_out_dim)
...
class QNetwork(nn.Module):
    ...
    Sim-to-Real Transfer of Robotic Control with Dynamics Randomization
    """
    def __init__(self, obs_dim, act_dim, hidden_dim, LSTM_out_dim, nOutputs = 10, activation=F.relu):
        super().__init__()

        self.activation = activation

        self.linear1 = nn.Linear(obs_dim+act_dim, hidden_dim)
        self.linear2 = nn.Linear(obs_dim+act_dim+LSTM_out_dim, hidden_dim)
        self.lstm1 = nn.LSTM(hidden_dim, LSTM_out_dim,batch_first=True)

        self.hidden_layer = nn.Linear(hidden_dim, hidden_dim)
        self.out_layer = nn.Linear(hidden_dim, nOutputs)
        # weights initialization
        #self.linear4.apply(linear_weights_init)
    ...
    def forward(self, state, action, last_action, hidden_in):
        ...
        x = torch.cat([state, action, x], -1) 

        x = self.activation(self.linear2(x))  # linear layer for 3d input only applied on the last dim
        #x = self.activation(self.hidden_layer(x))
        x = self.activation(self.hidden_layer(x))
        x = self.out_layer(x)
...
class SPDNetwork(nn.Module):
    ...
    Sim-to-Real Transfer of Robotic Control with Dynamics Randomization
    """
    def __init__(self, obs_dim, act_dim, hidden_dim, LSTM_out_dim, nOutputs = 1, activation=F.relu):
        super().__init__()

        self.activation = activation

        self.linear1 = nn.Linear(obs_dim+act_dim, hidden_dim)
        self.linear2 = nn.Linear(obs_dim+LSTM_out_dim, hidden_dim)
        self.lstm1 = nn.LSTM(hidden_dim, LSTM_out_dim,batch_first=True)
...
class PolicyNetwork(nn.Module):
    ...
    def forward(self, state, last_action, hidden_in, deterministic=False,with_logprob=True):
        """ 
        state shape: (batch_size, sequence_length, state_dim)
        ...
        return pi_action, logp_pi, lstm_hidden
...
class LSTMActorCritic(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim , act_limit, hidden_dim=64, LSTM_out_dim = 5, activation=F.relu):
        ...
        self.spd = SPDNetwork(obs_dim, act_dim, hidden_dim,LSTM_out_dim, nOutputs=1, activation=activation)
    ...
    def get_action(self, state, last_action, hidden_in, deterministic=False):
        with torch.no_grad():
            ...
            pi_action, _ , hidden_out =   self.pi(state, last_action, hidden_in , deterministic, False)
...
class SAC:
    ...
    def compute_loss_q(self,data, hidden_in):
        ...
        q, hidden_out = self.ac.q(state, action, last_action, hidden_in)
        
        with torch.no_grad():
            # Target actions come from *current* policy
            next_action, logp_a2, _ = self.ac.pi(next_state, action, hidden_out)

            # Target Q-values
            q_targ,_ = self.q_targ(next_state, next_action, action, hidden_out)
        ...
        target_q = 20
        #weight_update = list(loss_q.mean(dim=(1,2)).detach().numpy())
    ...
    def compute_loss_spd(self,data,hidden_in):
        ...
        spd_est, _ = self.ac.spd(state, last_action, hidden_in)
    ...
    def compute_loss_pi(self,data,hidden_in):
        ...
        pi, logp_pi, _ = self.ac.pi(state, last_action, hidden_in)

        q, _ = self.ac.q(state, pi, last_action, hidden_in)

        q_pi = torch.min(q,dim=-1,keepdim=True).values 
        # Entropy-regularized policy loss
        #print(q_pi.shape,q.shape)

        loss_pi = -(q_pi + self.alpha * (-logp_pi.unsqueeze(-1) - self.target_entropy) + self.beta * 2*pi).mean()
   ...
   # Update networks given a batch of experiences
    ...
    def get_speed_est(self,o,l_a,hin):
      ...
      o = torch.as_tensor(o, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
      l_a = torch.as_tensor(l_a, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
      spd_est,_ = self.ac.spd(o,l_a, hin)
      return [spd_est.detach().numpy().squeeze()*150+150, spd]
    ...
    def eval_agent(self, targetSpeeds, torques = None, freq = None,temp = None):
        ...
        for i in range(len(targetSpeeds)):
          ...
          actions.append(a*2)
          states.append(o)
          speeds.append(self.get_speed_est(o,a,hidden_in))
          l_a = a
