...
#            self.env = USM_EXP()
    ...
    def compute_loss_q(self,data):
        ...
        a_khz = 0.1*a.squeeze()*((o[:,-1]-o[:,-3])*150)
        ...
        with torch.no_grad():
            ...
            backup = r  + self.gamma * (q_pi_targ ) - self.beta*torch.abs(a_khz)
    ...
    def compute_loss_pi(self,data):
        ...
        pi_khz = 0.1*pi.squeeze()*((o[:,-1]-o[:,-3])*150)
   ...
   # Update networks given a batch of experiences
       ...
       # Main loop: collect experience in env and update/log each epoch
        for ep in range(1,num_eps):
           ...
           # Scale observation
            ...
            if ep % 10 == 0:
                print("Episode: {} --> Temp: {} --> TargetSpeed/Speed: {}/{} --> Torque: {}--> reward: {} --> Q0: {} --> alpha: {} --> beta: {}".format(ep,self.env.get_temp(),self.env.get_targetSpeed(), self.env.get_speed(), self.env.get_torque(),ep_ret,q*self.gamma,self.alpha.detach().numpy(),self.beta.detach().numpy()))
            
            # Save some logs
            LOGs.append(np.array([ep,self.env.get_temp(),self.env.get_torque(),self.env.get_targetSpeed(),ep_ret,self.env.get_speed(),q*self.gamma,self.alpha.detach().numpy(),self.beta.detach().numpy()]))
    ...
    def plot_eval(self,states, actions):
        ...
        ax22.set_ylabel('Optima Gain [kHz/rpm]',fontweight='bold',fontsize=16);
        ax2.legend();
        ax22.legend();
