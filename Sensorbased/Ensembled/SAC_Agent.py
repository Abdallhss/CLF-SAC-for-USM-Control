...
class SAC:
    ...
    def __init__(self, env_type = "SIM", obs_dim = 4, act_dim = 1, act_limit = [2],
                 ...
                 gamma=0.5, polyak=0.995, lr=1e-3, target_entropy = -4, seed=1,load_buffer = None):
...
#            self.env = USM_EXP()
    ...
    def compute_loss_q(self,data):
        ...
        a_w = (a[:,2].squeeze() + 1)/2
        a_khz = 2*a[:,0].squeeze()*a_w + 0.1*a[:,1].squeeze()*((o[:,-1]-o[:,-3])*150)*(1-a_w)
        ...
        with torch.no_grad():
            ...
            q_pi_targ = torch.min(q_pi_targ,dim=1).values - self.alpha * logp_a2 - self.beta*torch.abs(a_khz)
    ...
    def compute_loss_pi(self,data):
        ...
        pi_w = (pi[:,2].squeeze() + 1)/2
        pi_khz = 2*pi[:,0].squeeze()*pi_w + 0.1*pi[:,1].squeeze()*((o[:,-1]-o[:,-3])*150)*(1-pi_w)
        ...
        loss_pi = -(q_pi + self.beta*pi_khz + self.alpha * (-logp_pi - self.target_entropy)).mean()  
        ...
        loss_beta = self.log_beta * pi_khz.detach().mean()      
   ...
   # Update networks given a batch of experiences
    ...
    def run_evals_full(self):
        ...
        for torque in [0,0.2,0.5]:
            speeds = self.eval_agent(targetSpeeds,torques=[torque]*len(targetSpeeds),freq=39,temp=30)
        ...
        for targSpeed in [100,200,300]:    
            speeds = self.eval_agent([targSpeed]*len(torques),torques,freq=39,temp=30)
       ...
       #Step Torque -- [0-0.5] N.m
        ...
        for targspeed in [100,200,300]:
            for torque in [0.25,0.5]:
                ...
                speeds = self.eval_agent([targspeed]*len(torques),torques,freq=39,temp=30)
                np.savetxt('logs/conSpeed_{}rpm_sinTorque_{}Nm_{}.txt'.format(targspeed,torque,self.env_type), speeds, delimiter=',')
    ...
    def plot_eval(self,states, actions):
        ...
        ax1.plot(states[:,4],'r',label='Speed')
        ax1.plot(states[:,2],'b--',label='TargetSpeed')
        ax1.plot(abs(states[:,4]-states[:,2]), 'k',label = 'Speed Error')
        ...
        ax22 = ax2.twinx()
        ax22.plot(actions[:,1]*0.1,'r--',label='Freq Action')
        ax22.set_ylabel('Optimal Gain [kHz/rpm]',fontweight='bold',fontsize=16);
        ax2.legend();
        ax22.legend();
        #Plot 2
        ax3 = plt.subplot(2,2,3)
        ax3.plot(states[:,3], label='Temperature')
        ax3.set_xlabel('Step',fontweight='bold',fontsize=16);
        ax3.set_ylabel('Temperature [°C]',fontweight='bold',fontsize=16);
        
        ax33 = ax3.twinx()
        ax33.plot((states[:,1]),'r--',label='Load Torque')
        ax33.set_ylabel('Load Torque [N.m]',fontweight='bold',fontsize=16);
        ax3.legend();
        ax33.legend();

        ax4 = plt.subplot(2,2,4)
        ax4.plot(actions[:,0]*2, label='Freq Action')
        ax4.set_xlabel('Step',fontweight='bold',fontsize=16);
        ax4.set_ylabel('Frequency Action [kHz]',fontweight='bold',fontsize=16);

        ax44 = ax4.twinx()
        ax44.plot((actions[:,2]+1)/2,'r--',label='Action Weight')
        ax44.set_ylabel('Action Weight []',fontweight='bold',fontsize=16);
        ax4.legend();
        ax44.legend();
